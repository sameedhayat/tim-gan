...
def main():
    ...
    model.cuda()
    model.load(
        os.path.join(opts.output_dir, "model", opts.name, "%s.pth" % (opts.which_epoch))
    )
    model.eval()

    # create website
    web_dir = os.path.join(
        opts.result_dir, "%s_%s_%s" % (opts.name, opts.phase, opts.which_epoch)
    )
    webpage = html.HTML(
        web_dir,
        "Experiment = %s, Phase = %s, Epoch = %s"
        % (opts.name, opts.phase, opts.which_epoch),
    )
    ...
    for it, (src_img, text, tgt_img) in enumerate(loader):
        ...
        visuals = OrderedDict(
            [
                ("input", util.tensor2im(model.real_A[0].detach())),
                ("output" + ": " + text[0], util.tensor2im(model.fake_B[0].detach())),
                ("real_image", util.tensor2im(model.real_B[0].detach())),
                (
                    "attn_mask",
                    util.tensor2im(model.attn_dis[0].detach(), normalize=False),
                ),
                (
                    "gt_attn_mask",
                    util.tensor2im(model.gt_attn_dis[0].detach(), normalize=False),
                ),
            ]
        )
        img_path = [str(it)]
    ...
    webpage.save()
    np.savez(
        os.path.join(web_dir, "gate.npz"),
        gate=np.array(gate_out),
        text=np.array(alltexts),
        token=np.array(texttokens),
        attn1=np.array(textattn1),
        attn2=np.array(textattn2),
    )
    return
