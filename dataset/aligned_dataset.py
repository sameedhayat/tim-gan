...
class AlignedDataset(data.Dataset):
    def __init__(self, opts):
        ...
        # image names
        self.images_A = util.image_file(
            os.listdir(join(self.dataroot, opts.phase + "_A"))
        )
        self.images_A.sort()
        self.images_A = [
            join(self.dataroot, opts.phase + "_A", name) for name in self.images_A
        ]
        self.images_B = util.image_file(
            os.listdir(join(self.dataroot, opts.phase + "_B"))
        )
        self.images_B.sort()
        self.images_B = [
            join(self.dataroot, opts.phase + "_B", name) for name in self.images_B
        ]
    ...
    def __getitem__(self, index):
        ...
        resize_size = (
            int(self.crop_size[0] * resize_ratio),
            int(self.crop_size[1] * resize_ratio),
        )
